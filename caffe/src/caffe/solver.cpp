...
void Solver<Dtype>::Step(int iters) {
  ...
  */
  ...
  while (iter_ < stop_iter) {
    ...
    if (display) {
      LOG_IF(INFO, Caffe::root_solver()) << std::setprecision(8) << "Iteration " << iter_
          << ", loss = " << smoothed_loss_;
    ...
    if (iter_ % 100 == 0)
    {
        ...
        float avg_time = (float(cur_time - last_time) / CLOCKS_PER_SEC) / 100.0;
        last_time = cur_time;
        LOG_IF(INFO, Caffe::root_solver()) << "train speed: " << avg_time << "s / iter";
    }

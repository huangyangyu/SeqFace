...
void T_DSALossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
{
    ...
    //??this->param_propagate_down_.resize(this->blobs_.size(), bottom.size() != 3);
}

template <typename Dtype>
void T_DSALossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
{
    M_ = bottom[0]->num();
    ...
    // valid count
    //vector<int> valid_counts_shape(1);
    //valid_counts_shape[0] = M_;
    //valid_counts_.Reshape(valid_counts_shape);
    // loss
    //vector<int> loss_shape(2);
    //loss_shape[0] = M_;
    //loss_shape[1] = N_;
...
template <typename Dtype>
void T_DSALossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
{
    // Share parameter
    if (bottom.size() == 3 && bottom[2]->cpu_data() != this->blobs_[0]->cpu_data())
    {
        ...
        if (iter_ > diff_iter_)
        {
            for (int id_n = 0; id_n < ID_N_; id_n++)
            {
                ...
                if (label_value < faceid_num_ || n < faceid_num_)
                {
                    ...
                    //loss += (Dtype(1.0) - lambda_) * std::max(alpha_ * square_diff_data[m * N_ + label_value] - square_diff_data[m * N_ + n] + beta_, Dtype(0.0)) / ID_N_;
                }
            }
        }
...
template <typename Dtype>
void T_DSALossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
                                       ...
                                       const vector<Blob<Dtype>*>& bottom)
{
    ...
    if (this->param_propagate_down_[0])
    {
        for (int m_ = 0; m_ < M_; m_++)
        {
            ...
            for (int m = 0; m < M_; m++)
            {
                ...
                if (label_value == n)
                {
                    // important: ensure that n is the first time
                    if (m < m_)
                        break;
    ...
    {
        Dtype valid_count = Dtype(0.0);
        caffe_set(M_ * K_, Dtype(0.0), bottom_diff);//??
        for (int m = 0; m < M_; m++)
        {
            int label_value = static_cast<int>(label[m]);
            if (has_ignore_label_ && label_value == ignore_label_)
...
#ifdef CPU_ONLY
STUB_GPU(T_DSALossLayer);
#endif

INSTANTIATE_CLASS(T_DSALossLayer);
REGISTER_LAYER_CLASS(T_DSALoss);

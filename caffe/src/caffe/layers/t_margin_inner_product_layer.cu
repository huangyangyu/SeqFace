...
#include "caffe/layers/t_margin_inner_product_layer.hpp"
...
template <typename Dtype>
__global__ void T_Margin_double_forward_gpu(int nthreads, const int N_, Dtype lambda,
...
template <typename Dtype>
__global__ void T_Margin_triple_forward_gpu(int nthreads, const int N_, Dtype lambda,
...
template <typename Dtype>
__global__ void T_Margin_quadruple_forward_gpu(int nthreads, const int N_, Dtype lambda,
...
template <typename Dtype>
__global__ void T_Margin_bottom_double_backward_gpu(int nthreads, const int N_, const int K_, Dtype lambda,
...
template <typename Dtype>
__global__ void T_Margin_bottom_triple_backward_gpu(int nthreads, const int N_, const int K_, Dtype lambda,
            const Dtype* bottom, const Dtype* weight, const Dtype* top_diff, const Dtype* label,
...
template <typename Dtype>
__global__ void T_Margin_bottom_quadruple_backward_gpu(int nthreads, const int N_, const int K_, Dtype lambda,
            const Dtype* bottom, const Dtype* weight, const Dtype* top_diff, const Dtype* label,
...
template <typename Dtype>
void T_MarginInnerProductLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  // Share parameter
  if (bottom.size() == 3 && bottom[2]->gpu_data() != this->blobs_[0]->gpu_data())
  {
    this->blobs_[0]->ShareData(*(bottom[2]));
    this->blobs_[0]->ShareDiff(*(bottom[2]));
    // Fill the weights
  ...
  Dtype* norm_weight = this->blobs_[0]->mutable_gpu_data();
  ...
  case MarginInnerProductParameter_MarginType_TRIPLE:
    ...
    break;
  case MarginInnerProductParameter_MarginType_QUADRUPLE:
    T_Margin_quadruple_forward_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
...
template <typename Dtype>
void T_MarginInnerProductLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    case MarginInnerProductParameter_MarginType_SINGLE:
      ...
      break;
    case MarginInnerProductParameter_MarginType_DOUBLE:
      T_Margin_bottom_double_backward_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      ...
      break;
    case MarginInnerProductParameter_MarginType_TRIPLE:
      T_Margin_bottom_triple_backward_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, N_, K_, lambda_, bottom_data, weight, top_diff, label,
      ...
      break;
    case MarginInnerProductParameter_MarginType_QUADRUPLE:
      T_Margin_bottom_quadruple_backward_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, N_, K_, lambda_, bottom_data, weight, top_diff, label,
...
INSTANTIATE_LAYER_GPU_FUNCS(T_MarginInnerProductLayer);

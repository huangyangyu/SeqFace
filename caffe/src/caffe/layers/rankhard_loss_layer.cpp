...
void RankHardLossLayer<Dtype>::set_mask(const vector<Blob<Dtype>*>& bottom) {
    RankHardLossParameter rankhard_loss_param = this->layer_param_.rankhard_loss_param();
    int neg_num = rankhard_loss_param.neg_num();
    int pair_size = rankhard_loss_param.pair_size();
    float hard_ratio = rankhard_loss_param.hard_ratio();
    float rand_ratio = rankhard_loss_param.rand_ratio();
    float margin = rankhard_loss_param.margin();
    ...
    for(int i = 0; i < num; i += pair_size)
    {
        ...
        for(int j = 0; j < num; j++)
        {
            if(label[j] == label[i])
                continue;
            Dtype tloss = std::max(Dtype(0), dis_data[i * num + i + 1] - dis_data[i * num + j] + Dtype(margin));
            if(tloss == 0)
                continue;
        ...
        std::sort(negpairs.begin(), negpairs.end());
        for(int j = 0; j < neg_num; j++)
        {
            sid1.push_back(negpairs[j].second);
        }
        std::random_shuffle(negpairs.begin(), negpairs.end(), myrandom);
...
void RankHardLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    ...
    for(int i = 0; i < num; i += pair_size)
    {
        for(int j = 0; j < num; j ++)
        {
            ...
            Dtype tloss1 = std::max(Dtype(0), dis_data[i * num + i + 1] - dis_data[i * num + j] + Dtype(margin));
            Dtype tloss2 = std::max(Dtype(0), dis_data[i * num + i + 1] - dis_data[(i + 1) * num + j] + Dtype(margin));
            loss += tloss1 + tloss2;
        }
    }
    loss = loss / cnt;

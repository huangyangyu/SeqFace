...
void CenterLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < M_; i++) {
    ...
    if (has_ignore_label_ && label_value == ignore_label_) {
      caffe_set(K_, Dtype(0.0), distance_data + i * K_);
      continue;
    }
    // D(i,:) = X(i,:) - C(y(i),:)
...
void CenterLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0]) {
    ...
    // \sum_{y_i==j}
    caffe_set(N_ * K_, (Dtype)0., variation_sum_data);
    for (int n = 0; n < N_; n++) {
      int count = 0;
      for (int m = 0; m < M_; m++) {
    ...
    /*
    caffe_set(N_, 0 , count_data);
    for (int m = 0; m < M_; m++) {
      const int label_value = static_cast<int>(label[m]);
      if (has_ignore_label_ && label_value == ignore_label_) {
        continue;
      }
      caffe_sub(K_, variation_sum_data + label_value * K_, distance_data + m * K_, variation_sum_data + label_value * K_);
    ...
    for (int m = 0; m < M_; m++) {
      ...
      if (count_data[n] > 0)
      {
        caffe_axpy(K_, (Dtype)1./(count_data[n] + (Dtype)1.), variation_sum_data + n * K_, center_diff + n * K_);
        count_data[n] = 0;
      }
    }
    */
  ...
  if (propagate_down[0]) {
    ...
    caffe_scal(M_ * K_, top[0]->cpu_diff()[0] / valid_count, bottom[0]->mutable_cpu_diff());

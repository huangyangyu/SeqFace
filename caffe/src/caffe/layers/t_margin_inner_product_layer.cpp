...
void T_MarginInnerProductLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  N_ = this->layer_param_.margin_inner_product_param().num_output();
  temp_seqid_num_ = this->layer_param_.margin_inner_product_param().temp_seqid_num();
  ...
  if (this->blobs_.size() > 0) {
    ...
    weight_shape[0] = N_ + temp_seqid_num_;
    weight_shape[1] = K_;
...
template <typename Dtype>
void T_MarginInnerProductLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void T_MarginInnerProductLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  // Share parameter
...
template <typename Dtype>
void T_MarginInnerProductLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(T_MarginInnerProductLayer);
#endif

INSTANTIATE_CLASS(T_MarginInnerProductLayer);
REGISTER_LAYER_CLASS(T_MarginInnerProduct);

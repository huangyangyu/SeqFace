...
#ifdef USE_OPENCV
cv::Mat ReadImageRegionToCVMat(const string& filename,
  ...
  if (h > 0 && w > 0) {
    ...
    cv_img_origin = cv_img_origin(region);
  }
  if (height > 0 && width > 0) {
...
bool ReadImageRegionToDatum(const string& filename, const int label,
    const int y, const int x, const int h, const int w,
    const int height, const int width, const bool is_color,
    const std::string & encoding, Datum* datum) {
  cv::Mat cv_img = ReadImageRegionToCVMat(filename, y, x, h, w, height, width, is_color);
  if (cv_img.data) {
    if (encoding.size()) {
      if ( (cv_img.channels() == 3) == is_color && !height && !width &&
